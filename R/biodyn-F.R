...
setMethod( 'f',signature(object='biodyn'),
      function(object,tolVal=1e-5,niter=100,yield=TRUE){
        res=nr(C=catch(  object),
               F=harvest(object),
               B=stock(  object),
               r=params( object)["r"],K=params(object)["k"],
               tolVal=tolVal,niter=niter,yield=yield)
        res[["F"]]})

setGeneric('computeF', function(object,...) standardGeneric('computeF'))
setMethod( 'computeF',signature(object='biodyn'),
           function(object,tolVal=1e-5,niter=100,yield=TRUE){
             res=nr(C=catch(  object),
                    F=harvest(object),
                    B=stock(  object),
